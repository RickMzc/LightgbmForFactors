...
[Snap SH+SW parquet]          [Trade SH+SW parquet]       [Order SH+SW parquet]
  ...
  (3s bucket, sort, dedup)     (normalize, ceil bucket)     (decode SZ encoding)
        │                             │                           │
        ▼                             ▼                           ▼
  + date column injected       _aggregate_trades            _aggregate_orders
  LabelGenerator               (asof-join snap, tick agg)   (amount, count, depth)
  (mid_price → fwd returns)          │                           │
        │                             │                           │
        └──────────┬──────────────────┴───────────────┬──────────┘
                   ▼                                  ▼
             FeatureFactory.compute_many    _join_trade_stats / _join_order_stats
             (_time_group uses date)              │
                   │                                  │
                   ▼                                  ▼
...
**SZ 列名映射** (P1-#9 修复)：`_SZ_TO_SH` 是 SZ→SH 正向映射（rename 用），`_SH_TO_SZ` 是 SH→SZ 反向映射（scan 用）。scan 时用反向映射把统一 SH 列名转为 SZ parquet 列名；collect 后用正向映射把 SZ 列名 rename 为统一 SH 列名。两个方向都是正确的键→值查找。
...
**Clock-time 校验** (P0-#3 修复)：`shift(-5)` 不总是等于 15s — 当股票缺失快照时，shift 跳过的行可能跨越更长的时间。代码现在校验 `|fwd_ts - ts - horizon_secs| ≤ 3s`，超阈值标签置 NaN。
...
**⚠️ required_cols** (P1-#11 修复)：所有 trade 特征声明 `_TRADE_REQUIRED_COLS = ["UpdateTime", "BidPrice1", "AskPrice1", "mid_price"]`；所有 order 特征声明 `_ORDER_REQUIRED_COLS = ["BidPrice1", "AskPrice1", "mid_price"]`。`_join_trade_stats` 缺 `UpdateTime` 时直接 `raise KeyError`，不再伪造 `"09:30:00.000"`。
...
| 大单阈值未来函数（成交） | 同日 90% 分位 | 前日阈值缓存 `/fast1/user001/factor_values/_trade_thresh/`，无历史时 null |
| 大单阈值未来函数（委托） | 同日 90% 分位 | 前日阈值缓存 `/fast1/user001/factor_values/_order_thresh/`，无历史时 null |
| 委托分桶未来函数 | `floor(s/3)*3` | `_ceil_3s(_parse_time_float(OrderTime))` |
| 委托深度 join 错位 | 同 bucket exact join | tick-level `join_asof(strategy="backward")` |
| 委托深度混入撤单 | 撤单也进 depth 统计 | `valid_buy/sell = is_new & ...` 只统计新增委托 |
...
| `preprocess(df)` | 截面 Z-score 标准化：每 `(date, timestamp)` 分组内 `(x-mean)/std`，std≤1e-6 的截面 clamp 到 0 |
| `prepare_data(df)` | 提取 X, y, stock_ids（从全局 `stock_id` 列读取，不再自行编码），过滤 NaN |
...
**⚠️ 关键设计**：
- `preprocess` 不再跨 split 共享统计量 — 截面 Z-score 天然按日独立，train/val/test 各自计算自己的 `(date, timestamp)` 统计量；`std ≤ 1e-6` 的截面 clamp 到 0 防数值爆炸
- `stock_id` 必须由调用方在 `full_df` 上一次性生成（`SecurityID.cast(Categorical).to_physical()`），确保 train/val/test 拆分后映射一致
- `group_by(["date", "timestamp"])` 杜绝多日同一时刻被揉进同一截面的"跨日大锅炖"
- `prepare_data` (P0-#4 修复) 只 drop target + key 列 (`date`, `timestamp`, `SecurityID`) 的 null，特征 null 交给 LightGBM 原生 missing-value 处理。深市不再因 `MaxDurPressure` 被整批剔除
...
| 方法 | 功能 |
|------|------|
| `rank_ic_per_timestamp(df, pred_col, label_col)` | 每个 `(date, timestamp)` 截面内计算 Spearman ρ（P0-#5：date-aware 防跨日混截面） |
| `evaluate(predictions, labels, timestamps, security_ids, dates)` | 完整评估，dates 用于按日分组 |
| `evaluate_multi_horizon(..., dates)` | 多 horizon 同时评估，传递 dates |
...
3. 立即注入 date 列 ← 关键：让 _time_group() 能防跨日污染
4. LabelGenerator.generate(df)                     → 加 mid_price + 5 个 horizon 标签
5. FeatureFactory.compute_many(df, features, date)  → 加 75 个特征列
6. 返回 df + 元信息
```
...
2. concat 为全量 full_df
3. 一次性生成全局 stock_id (SecurityID → Categorical → to_physical) ← 关键：防映射错乱
4. create_walk_forward_splits(dates)
5. AlphaModel.fit_walk_forward(df, splits)
6. CrossSectionalEvaluator 评估每个 fold
```
...
10. 大单全天阈值(成交) → 已改前日缓存，无历史时 null
11. 大单全天阈值(委托) → 已改前日缓存 `/fast1/user001/factor_values/_order_thresh/`
12. 成交 floor 分桶 → 已改 ceil
13. 穿透同桶 join → 已改 asof backward
14. 委托 floor 分桶 → 已改 ceil (`_ceil_3s(_parse_time_float(OrderTime))`)
15. 委托深度同桶 join → 已改 tick-level `join_asof(strategy="backward")`
16. 委托深度混入撤单 → `valid = is_new & is_buy/is_sell & ...`
17. 截面 Z-score 跨日大锅炖 → `group_by(["date","timestamp"])` + std 守卫
18. stock_id `to_physical()` 逐 DataFrame 映射错乱 → 全局一次性编码
19. `prepare_data` drop_nulls 系统性删除深市 → 只 drop target+key 列，特征 null 交 LGB
20. Rank-IC evaluation 跨日混截面 → `group_by(["date","timestamp"])`
21. Label row-shift 不校验时间差 → clock-time gap `|fwd_ts - ts - horizon| ≤ 3s`
22. `date` 列注入太晚致 `_time_group()` 成死代码 → 数据加载后立即注入
23. `TradeAggregator` 含 floor 分桶未来函数 → 整类删除
24. SZ 列名映射方向错误 (P1-#9) → `_SH_TO_SZ` 反向映射 scan + `_SZ_TO_SH` 正向 rename
25. 25 个 trade/order 特征 `required_cols=[]` → 全部声明 `_TRADE_REQUIRED_COLS` / `_ORDER_REQUIRED_COLS`
26. `_snap_time` 伪造 `"09:30:00.000"` 回退 → 缺列直接 `raise KeyError`
27. `TradeIntensityZ` / OFI PCA 多日跨截面 (P1-#8) → date-aware group_by
28. `IsGapLimitDown` join key 硬编码 (P1-#7) → `on=g_key`
