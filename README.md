...
**⚠️ 已删除**：`TradeAggregator` 类已废弃删除。旧版使用 `floor(s/3)*3` 向下取整分桶存在未来函数，正确的逐笔聚合逻辑在 `feature_factory.py` 中（ceil bucketing + asof-join backward）。
...
**⚠️ 关键设计**：
- `preprocess` 不再跨 split 共享统计量 — 截面 Z-score 天然按日独立，train/val/test 各自计算自己的 `(date, timestamp)` 统计量
- `stock_id` 必须由调用方在 `full_df` 上一次性生成（`SecurityID.cast(Categorical).to_physical()`），确保 train/val/test 拆分后映射一致
- `group_by(["date", "timestamp"])` 杜绝了多日同一时刻被揉进同一截面的"跨日大锅炖"
